...
(defn larger-bid?
  "Given a first bid, returns true if the second bid is larger; otherwise,
   false."
...
(defn ordered-indexes
  "Given a current index and a count of players, returns the next indexes of
   all future players."
...
(defn first-idx-with-dice
  "Given a collection of players and an ordered index collection, returns the
   index of the first player with dice."
  [players idxs]
  (loop [idx (first idxs)
         ordered-indexes idxs]
    (if (< 0 (count (get-in players [idx :dice])))
      idx
      (recur (first ordered-indexes) (rest ordered-indexes)))))
...
(defn next-player-idx
  "Give a collection of players and current player idx, returns the next
   active player's idx."
  [players current-player-idx]
...
(defn challenge
  "Given a collection of players, the current bid, and the index of the
   challenger, returns a map indicating the success/failure of the attempt,
   which player idx should be penalized, and the total of bid rank."
...
(defn game-over?
  "Given a collection of players, returns true if only one player has dice in
   their collection; otherwise, false."
  [players]
  (let [dice-counts (map count (map :dice players))]
    (= 1 (count (filter #(< 0 %) dice-counts)))))

(defn initialize-player
  "Given a raw player and a number of dice, returns a map representing the
...
(defn with-penalized-player
  "Given a collection of players and potential penalization index, returns the
   players collection with the penalty applied."
...
(defn with-rewarded-player
  "Given a collection of players and a potential penalization index, returns
   the players with the reward applied."
...
(defn initialize-game
  "Given a map containing player info and a number of dice to use, initializes
   a game state using that info."
...
(defn new-bid
  "Given a game-state, quantity, and rank, returns a new game state with
   either the updated bid or error message."
...
(defn exact-bid
  "Given a game-state, returns a new game state with the challenge result and
   messaging."
...
(defn boot-player
  "Given a game state and a player idx, re-initializes the current round
   without the player."
